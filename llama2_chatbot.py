...
selected_option = st.sidebar.selectbox('Escoge tu modelo LLaMA2:', ['LLaMA2-13B', 'LLaMA2-7B'], key='model')
if selected_option == 'LLaMA2-7B':
    st.session_state['llm'] = REPLICATE_MODEL_ENDPOINT7B
else:
    st.session_state['llm'] = REPLICATE_MODEL_ENDPOINT13B
#Model hyper parameters:
st.session_state['temperature'] = st.sidebar.slider('Temperatura:', min_value=0.01, max_value=5.0, value=0.1, step=0.01)
st.session_state['top_p'] = st.sidebar.slider('Top P:', min_value=0.01, max_value=1.0, value=0.9, step=0.01)
st.session_state['max_seq_len'] = st.sidebar.slider('Longitud máxima de secuencia:', min_value=64, max_value=4096, value=2048, step=8)


# Add the "Clear Chat History" button to the sidebar
clear_chat_history_button = st.sidebar.button("Limpia el historial de tu chat")
...
text1 = 'Chatbot Demo Codigo' 
text2 = 'Model en Replicate' 
...
# Accept user input
if prompt := st.chat_input("Escribe tu pregunta aquí para hablar con LLaMA2"):
